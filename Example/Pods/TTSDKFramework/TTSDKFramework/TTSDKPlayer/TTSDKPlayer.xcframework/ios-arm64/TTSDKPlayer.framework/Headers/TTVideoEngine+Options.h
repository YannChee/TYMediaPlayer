 ...
 * @brief 各种 option 定义
 * @notes <br>
 *  - 使用以下定义的 key 来设置 或 获取 各种播放 option。 <br>
 *  - 如果要获取opiton值，请使用这个宏 VEKKEY(key) 来生成key， 例如获取是否使用硬解：VEKKEY(VEKKeyPlayerHardwareDecode_BOOL)
 */
/**
 * @locale en
 * @hidden
 ...
 * @notes <br>
 *        - Use the defined keys below to set or get various playback options.<br>
 *        - To get the option value, use the macro VEKKEY(key) to generate the key. For example, to check if hardware decoding is used: VEKKEY(VEKKeyPlayerHardwareDecode_BOOL)
 */
typedef NS_ENUM(NSInteger, VEKKey)
{
    ...
    VEKKeyIsBmfFlexSrMode                       = 1042,
    VEKKeyIsEnableStreamInaccStartOpt_BOOL      = 1043,
    ...
    /// set mst m3u8 bitrate for switch
    VEKKeySetMasterm3u8VideoBandwidth_NSInteger      = (1 << 14) + (1 << 13) + 12,
    /// request subtitle use mdl proxy url
    VEKKeySubTitleEnableMDL_BOOL                     = (1 << 14) + (1 << 13) + 13,
    /// enable mp4 custom buffer process
    VEKKeyMp4CustomBufferProcessEnable_BOOL          = (1 << 14) + (1 << 13) + 14,
    
    VEKKeyPlayerIsSimulator_BOOL                     = (1 << 14) + (1 << 13) + 15,
    ///
    ///
    ///tob end
    ///
    ...
    VEKKeyIsHandleResolutionChange              = 40054,
    ...
    VEKKeyPlayerEnableCPPh266CodecOpt_BOOL   = 40073,
    VEKKeyIsAudioPtsMSCorrectionExtension       = 40074,
    VEKKeyIsVC2WppMode                          = 40075,
    VEKKeyIsh266LowLatency                   = 40076,
    ...
    VEKKeyIsVOutletPreRenderOpt                   = 42095,
    VEKKeyIsHDRAutomaticIdentification            = 42116,
    VEKKeyIsEnableOptInaccStart                   = 42119,
    VEKKeyIsEnableOptSeekClk                      = 42120,
	VEKKeyIsEnableHlsPtsRecalOpt                  = 42140,
    ...
    VEKKeyIsEnableNewBMFSRInterface               = 42801,
    VEKKeyKeyIsEnableSoftDecodeHDR                = 42856,
    VEKKeyIsEnableSubRequestHeader                = 46497,
    VEKKeyIsEnableSeekLastFrame                   = 46498,
    VEKKeyIsSeekEndFFMPEGThresholdSI              = 46499,
    VEKKeyIsAllowBufferProcessProtoNameSI         = 46500,
    VEKKeyIgnoreFMP4DRMSeek                       = 46501,
    ...
    VEKKeyPlayerSetPrivateFFmpegProto_NSString = 70013,
    VEKKeyPlayerSetHLSConvertUrlOrder_NSString = 70014,
 ...
 * @hidden
 * @type keytype
 * @brief 播放器各种 option 定义
 * @notes <br>
 *  - 请使用这个 VEKKEY(key) 来生成 key，例如： VEKKEY(VEKGetKeyPlayerVideoWidth_NSInteger)
 */
/**
 * @locale en
 * @hidden
 * @type keytype
 * @brief Use these keys, only get info, Please use marco VEKKEY(key), example: VEKKEY(VEKGetKeyPlayerVideoWidth_NSInteger)
 * @notes <br>
 *        - Please use this VEKKEY(key) to generate the key, for example: VEKKEY(VEKGetKeyPlayerVideoWidth_NSInteger)
 */
 ...
 * @hidden
 * @type keytype
 * @brief TTVideoEngine 全局静态配置
 */
/**
 * @locale en
 * @type keytype
 * @brief TTVideoEngine Key about global static settings
 * @detail keytype
 * @hidden 
 */
typedef NS_ENUM(NSInteger, VEKGSKey)
{
    ...
    VEGSKeyAlgoOptionEnableMP3Support = 50019,
    VEGSKeyAlgoOptionCacheSelectedRep = 50020,
    VEGSKeyAlgoOptionFeatureContainCurrentByDefault = 50021,
    VEGSKeyAlgoOptionLimitRangeLogCount = 50024,
    VEGSKeyAlgoOptionRunnerWaitTimeoutMs = 50025,
    VEGSKeyAlgoOptionAllowParallelPreload = 50026,
    VEGSKeyAlgoOptionEnableRangeLogUpload = 50027,
    VEGSKeyAlgoOptionEnableDashSidxInfo = 50028,
    VEGSKeyAlgoOptionnIOControlEnableFileHash = 50033,
    VEGSKeyAlgoOptionSwitchMDLCache = 50035,
    VEGSKeyAlgoOptionAppGetCustomNetworkSpeed = 50038,
    VEGSKeyAlgoOptionUseContentLength = 50039,
    VEGSKeyAlgoOptionSelectPreloadRep = 50041,
    VEGSKeyAlgoOptionAsyncStartModule = 50042,
    VEGSKeyAlgoOptionEnableIODemand = 50044,
    VEGSKeyAlgoOptionLimitWaitingTaskNum = 50049,
    VEGSKeyAlgoOptionOptimizeFocusMedia = 50050,
    VEGSKeyAlgoOptionOptEngineMediaSeq = 50051,
    VEGSKeyAlgoOptionPlaceHolderMedia = 50052,
    VEGSKeyAlgoOptionOnlyAddMediaToMap = 50053,
    VEGSKeyAlgoOptionMediaListsCallbackSwitchToMainThread = 50054,
    VEGSKeyAlgoOptionPreloadFilterLiveMedia = 50055,
    VEGSKeyAlgoOptionRetryGetMediaLists = 50056,
    VEGSKeyAlgoOptionKeepCurrentMediaWhenRemoveAllMedias = 50057,
    VEGSKeyAlgoOptionRemoveAllMediasWhenUpdateMedias = 50058,
    VEGSKeyAlgoOptionDisablePlayloadIfBuffered = 50059,
    VEGSKeyAlgoOptionDisableSelectBitratePlaceHolder = 50060,
    VEGSKeyAlgoOptionSelectMediaOptimize = 50061,
    VEGSKeyAlgoOptionCheckPlayTraceId = 50062,
    VEGSKeyAlgoOptionEnableIOCtlStrategy = 50063,
    VEGSKeyAlgoOptionRetryUpdateTaskState = 50064,
    VEGSKeyAlgoOptionRangeBufferGetWithMediaId = 50065,
    VEGSKeyAlgoOptionDelayMsToGetMediaListsWhenRenderStart = 50066,
    VEGSKeyAlgoOptionEnableStrategyPerfOptimize = 50067,
    VEGSKeyAlgoOptionDefaultBitrateType = 50068,
    VEGSKeyAlgoOptionAutoResumeTaskWhenClose = 50069,
    VEGSKeyAlgoOptionSendFakeAllPreloadFinishMsg = 50071,
    VEGSKeyAlgoOptionEnableMediaListPreloadLog = 50072,
    VEGSKeyAlgoOptionMediaListCacheLogBackward = 50073,
    VEGSKeyAlgoOptionMediaListCacheLogForward = 50074,
    
    VEGSKeyAlgoOptionUseEngineNetworkTimeout = 50080,
    
    VEGSKeyAlgoOptionIndividualModuleSwitchBegin = 56000,
    VEGSKeyAlgoOptionEnableModulePlayload = 56001,
    VEGSKeyAlgoOptionEnableModuleSelectBitrate = 56002,
    VEGSKeyAlgoOptionEnableModuleBandwidth = 56003,
    VEGSKeyAlgoOptionEnableModulePreload = 56004,
    VEGSKeyAlgoOptionEnableModulePlayBuffer = 56005,
    VEGSKeyAlgoOptionEnableModuleDownloadIo = 56006,
    VEGSKeyAlgoOptionEnableModulePlayRange = 56007,
    VEGSKeyAlgoOptionEnableModuleDasp = 56008,
    VEGSKeyAlgoOptionEnableModuleAbr = 56009,
    VEGSKeyAlgoOptionModuleSR = 56010, // VCKeyOptionEnableModuleSR = 56010,
    
    VCKeyAlgoStringBegin = 59000,
    VEGSKeyAlgoStringPCDNPriority = 59001,
    VEGSKeyAlgoStringQUICPriority = 59002,
    VEGSKeyAlgoStringAPPCustomBandwidth = 59003,
    VEGSKeyAlgoStringSelectPortraitNeed = 59004,
    VEGSKeyAlgoStringIOCtlPC = 59005,
    VEGSKeyAlgoStringIOCtlN80 = 59006,
    VEGSKeyAlgoStringAbr = 59007,
    
    VEGSKeyAlgoGlobalOptionEnd = 60000,      /// TTVideoEngineStrategyAlgoGlobalOptionEnd
    
    VEGSKeyEngineGlobalOptionStart = 70000, //70000-79999 TTVideoEngine配置项
    VEGSKeyEngineGlobalOptionDetectPrivateField_BOOL = 70001, //检测Header中隐私字段开关【默认关闭】
    VEGSKeyEngineGlobalOptionEnd = 79999,
 ...
 * @hidden
 * @type keytype
 * @brief videoprocessor key value
 */
/**
 * @locale en
 * @type keytype
...
typedef NS_ENUM(NSInteger, VEKProcessorOptions) {
    ...
    VEKProcessorFilterBMFUpdateSRType = 29,
 ...
 * @hidden
 */
static NSString *const VEKProcessorGeneralValueStr = @"kProcessorGeneralValue";
/**
 * @locale zh
 * @hidden
 */
 ...
 * @hidden
 */
extern NSString *const VEKProcessorBMFSPAlgoType;
/**
 * @locale zh
 * @hidden
 */
extern NSString *const VEKProcessorBmfSharpWt;
/**
 * @locale zh
 * @hidden
 */
extern NSString *const VEKProcessorBmfSharpThr;
/**
 * @locale zh
 * @hidden
 */
extern NSString *const VEKProcessorBmfSharpOvrt;
/**
 * @locale zh
 * @hidden
 */
extern NSString *const VEKProcessorBmfSharpLcWtThr;
/**
 * @locale zh
 * @hidden
 */
extern NSString *const VEKProcessorBmfSharpG0;
/**
 * @locale zh
 * @hidden
 */
extern NSString *const VEKProcessorBmfSharpAdaptive;
/**
 * @locale zh
 * @hidden
 */
extern NSString *const VEKProcessorBmfSharpFilterBMFSPSync;
/**
 * @locale zh
 * @hidden
 */
extern NSString *const VEKProcessorBmfSharpFilterAsyncInit;
/**
 * @locale zh
 * @hidden
 */
extern NSString *const VEKProcessorBmfSharpFilterNewBMFInterface;
/**
 * @locale zh
 * @hidden
 */
extern NSString *const VEKProcessorFilterBMFSRAlgorithModel;
/**
 * @locale zh
 * @hidden
 */
extern NSString *const VEKProcessorFilterBMFFlexSrMode;

/**
 * @locale zh
 * @type api管理播放器实例 Options
 * @hidden (iOS)
 */
/**
 * @locale en
 ...
 * @param key 请使用 VEKKEY(key) 宏来生成key。
 * @return 返回指定option值。
 */
/**
 * @locale en
 * @hidden
 * @type api
 * @brief Get option that you care about.
 ...
 * @hidden
 * @type api
 * @brief 设置播放 opiton 参数
 * 例如：设置是否支持seek到最后一帧可以参考以下代码
 * [self setOptions:@{VEKKEY(VEKKeyPlayerSeekEndEnabled_BOOL):@(YES)}];
                      |                   |          |                          |
                Generate key            Field     valueType                   value
 * @param options key-value 结构。
 */
/**
 * @locale en
 * @hidden
 * @type api
 * @brief Set options by VEKKey
 * Example:
 * [self setOptions:@{VEKKEY(VEKKeyPlayerSeekEndEnabled_BOOL):@(YES)}];
                      |                   |          |                          |
                Generate key            Field     valueType                   value
