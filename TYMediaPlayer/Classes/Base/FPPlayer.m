...
static NSMutableDictionary <NSString* ,NSNumber *> *mylayRecords;
...
- (void)setupFPPlayerBlocks {
    ...
    self.currentPlayerManager.playerReadyToPlay = ^(id<FPPlayerMediaPlayback>  _Nonnull asset, NSURL * _Nonnull assetURL) {
        ...
        // playback 类别：1.表明应用以媒体播放为主要目的（如音乐/视频播放器）2. 允许静音开关开启时仍有声音
        [[AVAudioSession sharedInstance] setCategory:AVAudioSessionCategoryPlayback withOptions:AVAudioSessionCategoryOptionAllowBluetooth error:nil];
        // 1.* 声明当前应用的音频会话优先级 * 接管系统音频控制权（可能中断其他应用的音频播放）
...
- (void)setRate:(float)rate {
    self.currentPlayerManager.rate = rate;
}
